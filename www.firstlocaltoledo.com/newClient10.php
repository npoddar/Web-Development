...
while(empty($_POST['newDate'])){
    var_dump($_POST['newDate']);
	sleep(1);
}

$appStartDate = strip_tags($_POST['newDate']);
$running_day = date('w', strtotime($appStartDate)); // for given date

	$html = '';
	$time = '';
	$timeblocks = mysql_query(" SELECT tb.* FROM (SELECT tb.* FROM `timeblocks` as tb WHERE tb.date_from <= '{$appStartDate}' AND tb.date_to >= '{$appStartDate}' AND tb.weekDayNum = (DAYOFWEEK('{$appStartDate}')-1) AND tb.slots > (SELECT COUNT(*) FROM `appointments` WHERE `appointmentDate` = '$appStartDate' AND `timeblockId` = tb.timeblockID) AND `enabled` = 1 ORDER BY tb.priority DESC) as tb GROUP BY tb.startTime ORDER BY HOUR(tb.startTime) ASC ");
	while($timeblock = mysql_fetch_array($timeblocks)) {
		//$time .= $timeblock['timeblockID'].'">'.date("g:i", strtotime($timeblock['startTime'])).'-'.date("g:i", strtotime($timeblock['endTime']))
		//echo $time;
		$time .= date("g:i", strtotime($timeblock['startTime'])).'-'.date("g:i", strtotime($timeblock['endTime']))." " ;
...
else if($_POST['timefunction'] == 'false'){
	$_SESSION['message'] = '<h3 class="errorHdr">Time slot already taken</h3><br />';
	$clientQuery = '';
	$appointmentQuery = '';
	echo "Entered case timefunction false";	
}
else {
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    ...
    document.getElementById("timefunction").innerHTML=output;
	console.log("All times " + output);
	
	//$timeblockID = ($_POST['timeblockID']);
	//console.log($timeblockID);
	
	var e = document.getElementById("timeblockID");
	var strUser = e.options[e.selectedIndex].text;
	console.log("Selected time " + strUser);	
	
	pos = output.search(strUser);
	console.log("pos");
	console.log(pos);	
		
		if (pos == -1){
		
			document.getElementById("timeblockID").innerHTML=output ;
			document.getElementById("timefunction").value = 'false';
			return false;
		}
		
		else {
			document.getElementById("timefunction").value = 'true';
			console.log("Date exists");
			return true;
		}

	
	//selectedtime = document.getElementById("timeblockID").innerHTML;
	//console.log("Selected time");
	//console.log(selectedtime);
	
	}
  }
xmlhttp.open("GET","_times.php?eventStartDate1="+startDate);
xmlhttp.send();
...
xmlhttp1.onreadystatechange=function()
  ...
  if (xmlhttp1.readyState==4 && xmlhttp1.status==200)
    {
   		alltimes = xmlhttp1.responseText;
		//document.write(alltimes);
		console.log(alltime);
		
		console.log('line 207');
		selectedtime = document.getElementById("timeblockID").innerHTML; //all times//
		//document.write(selectedtime);
		console.log(selectedtime);
				
		pos = alltimes.search(selectedtime);
		
		if (pos == -1){
		
			document.getElementById("timeblockID").innerHTML=alltimes ;
			document.getElementById("timefunction").innerHTML = 'false';
			
			
		}
		
		else {
			document.getElementById("timefunction").innerHTML = 'true';
		}
	
    }
  }
xmlhttp1.open("GET","_times.php?eventStartDate="+startDate);
xmlhttp1.send();
}
...
<table width="100%" border="0">
  ...
  <tr>
    ...
    <td width="73%"><input type="button" name="update" id="update" value="Update" onclick="lock()" /></td>
  </tr>
</table>
...
<span id="btnMsg"></span>
<input name="addClientBtn" id="addClientBtn" type="submit" onclick="lock()" value="Enter Information" />
</fieldset>

<input id="timefunction" type="text" name="timefunction">
